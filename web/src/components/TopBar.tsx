...
export function TopBar({ updatedAt, busy, onRefresh }: TopBarProps) {
  ...
  return (
    <header className="flex items-center gap-4 pb-6">
      <span className="flex items-center gap-2 rounded-full border border-hairline bg-panel px-3 py-1.5 text-xs font-medium text-lime">
      ...
      <div className="ml-auto flex items-center gap-4">
        ...
        <button
          onClick={onRefresh}
          className="rounded-full bg-lime px-4 py-1.5 text-xs font-medium text-lime-ink transition-colors duration-150 hover:bg-[#047036]"
        >
          Refresh ▸
        </button>
      </div>
